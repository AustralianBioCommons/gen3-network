...
if (!configPath) throw new Error("Missing CDK context key: config");
...
for (const [k, v] of Object.entries(config.tags ?? {})) cdk.Tags.of(app).add(k, v);

for (const stageConfig of config.stages) {
  const resolved = resolveStageConfig(config, stageConfig);
  new NetworkStage(app, stageConfig.id, {
    env: { account: resolved.envTarget.account, region: resolved.envTarget.region },
